  ...
  return (
    <div style={{ userSelect: 'none' }} className={cx(flexStyle.fsbc, flexStyle.container)}>
      <Tooltip mouseEnterDelay={0} title={<div className={cx(fontFamilyStyle.fontFamily)}>
        <div>{(getData?.startTime) ? removeMilli(milliTimeToStringTime(stringTimeToMilliTime(getData.startTime))) : ''}</div>
      </div>} >
        <div className={timeAndPriceStyle.timeColor}>{(getData?.analyseTime) ? removeMilli(milliTimeToStringTime(stringTimeToMilliTime(getData.analyseTime))) : ''}</div>
      </Tooltip>
